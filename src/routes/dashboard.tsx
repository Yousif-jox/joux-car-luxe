...
import {
  loadCars, saveCars, loadRentals, formatEGP,
  type Car, type CarStatus, type FuelType, type ListingType, type RentalRecord
} from "@/lib/cars";
...
const STATUSES: CarStatus[] = ["متاح", "محجوز", "مباع", "مؤجر"];
const LISTING_TYPES: ListingType[] = ["بيع", "إيجار", "بيع وإيجار"];

const emptyForm = {
  brand: "", model: "", year: new Date().getFullYear(), price: 0,
  color: "", fuel: "بنزين" as FuelType, status: "متاح" as CarStatus,
  image: "", listingType: "بيع" as ListingType,
  rentalDay: 0, rentalWeek: 0, rentalMonth: 0,
};
...
function Dashboard() {
  ...
  useEffect(() => { if (sessionStorage.getItem("jox_auth") === "1") setAuthed(true); }, []);
  useEffect(() => {
    if (authed) {
      setCars(loadCars());
  ...
  const stats = useMemo(() => ({
    ...
    rented:    cars.filter(c => c.status === "مؤجر").length,
    value:     cars.reduce((s, c) => s + c.price, 0),
    rentalRevenue: rentals.reduce((s, r) => s + r.totalPrice, 0),
  }), [cars, rentals]);
  ...
  function submit(e: React.FormEvent) {
    ...
    const hasRental = form.listingType === "إيجار" || form.listingType === "بيع وإيجار";
    const rental = hasRental && form.rentalDay > 0 ? {
      pricePerDay: Number(form.rentalDay),
      pricePerWeek: Number(form.rentalWeek),
      pricePerMonth: Number(form.rentalMonth),
    } : undefined;

    const carData: Omit<Car, "id"> = {
      brand: form.brand, model: form.model,
      year: Number(form.year), price: Number(form.price),
      color: form.color, fuel: form.fuel, status: form.status,
      image: form.image, listingType: form.listingType, rental,
    };

    if (editId) {
      persist(cars.map(c => c.id === editId ? { ...c, ...carData } : c));
      setEditId(null);
    } else {
      persist([...cars, { id: Date.now().toString(), ...carData }]);
    }
    setForm({ ...emptyForm });
  }
  ...
  function edit(c: Car) {
    setEditId(c.id);
    setForm({
      brand: c.brand, model: c.model, year: c.year, price: c.price,
      color: c.color, fuel: c.fuel, status: c.status, image: c.image || "",
      listingType: c.listingType,
      rentalDay:   c.rental?.pricePerDay   || 0,
      rentalWeek:  c.rental?.pricePerWeek  || 0,
      rentalMonth: c.rental?.pricePerMonth || 0,
    });
  ...
  const showRentalFields = form.listingType === "إيجار" || form.listingType === "بيع وإيجار";
    ...
    <div className="min-h-screen">
      ...
      <main className="container mx-auto px-6 py-8 space-y-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard label="مؤجر حالياً" value={stats.rented} accent="blue" />
          <StatCard label="عقود إيجار" value={rentals.length} />
          <StatCard label="إيرادات الإيجار" value={formatEGP(stats.rentalRevenue)} highlight />
        ...
        <div className="flex gap-3">
          <button onClick={() => setTab("cars")}
            className={`px-5 py-2.5 rounded-xl text-sm font-medium border transition ${tab === "cars" ? "gold-gradient text-background border-transparent" : "border-border hover:border-gold/50"}`}>
            🚗 السيارات
          </button>
          <button onClick={() => setTab("rentals")}
            className={`px-5 py-2.5 rounded-xl text-sm font-medium border transition ${tab === "rentals" ? "gold-gradient text-background border-transparent" : "border-border hover:border-gold/50"}`}>
        ...
        {tab === "cars" && <>
          ...
          <section className="card-luxury rounded-2xl p-6">
            ...
            <form onSubmit={submit} className="grid md:grid-cols-3 lg:grid-cols-4 gap-4">
              ...
              <Field label="سعر البيع (ج.م)"><input type="number" required value={form.price} onChange={e => setForm({ ...form, price: Number(e.target.value) })} className={inputCls} /></Field>
              ...
              <Field label="نوع الإتاحة">
                <select value={form.listingType} onChange={e => setForm({ ...form, listingType: e.target.value as ListingType })} className={inputCls}>
              ...
              {/* حقول الإيجار */}
              {showRentalFields && <>
                <Field label="سعر الإيجار / يوم">
                  <input type="number" value={form.rentalDay} onChange={e => setForm({ ...form, rentalDay: Number(e.target.value) })} className={inputCls} />
                </Field>
                <Field label="سعر الإيجار / أسبوع">
                  <input type="number" value={form.rentalWeek} onChange={e => setForm({ ...form, rentalWeek: Number(e.target.value) })} className={inputCls} />
                </Field>
                <Field label="سعر الإيجار / شهر">
                  <input type="number" value={form.rentalMonth} onChange={e => setForm({ ...form, rentalMonth: Number(e.target.value) })} className={inputCls} />
                </Field>
              </>}
            ...
            <div className="space-y-3">
              {brandCounts.map(([b, n]) => (
                <div key={b} className="flex items-center gap-4">
                  <div className="w-28 text-sm font-semibold">{b}</div>
                  <div className="flex-1 h-8 bg-secondary rounded-lg overflow-hidden">
                    <div className="h-full gold-gradient rounded-lg flex items-center justify-end px-3 text-xs font-bold text-background transition-all"
                      style={{ width: `${(n / maxBrand) * 100}%` }}>{n}</div>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-secondary/50 text-xs uppercase text-muted-foreground">
                  <tr>
                    {["الماركة", "الموديل", "السنة", "السعر", "اللون", "الوقود", "النوع", "الحالة", "إيجار/يوم", "إجراءات"].map(h =>
                ...
                <tbody>
                  {cars.map(c => (
                    <tr key={c.id} className="border-t border-border/50 hover:bg-secondary/30 transition">
                      ...
                      <td className="px-4 py-3 text-gold font-semibold whitespace-nowrap">{formatEGP(c.price)}</td>
                      <td className="px-4 py-3">{c.color}</td>
                      <td className="px-4 py-3">{c.fuel}</td>
                      <td className="px-4 py-3">
                        <span className="px-2 py-0.5 rounded-full text-xs border border-gold/30 text-gold whitespace-nowrap">{c.listingType}</span>
                      </td>
                      <td className="px-4 py-3">{c.status}</td>
                      <td className="px-4 py-3 text-blue-400 whitespace-nowrap">
                        {c.rental ? formatEGP(c.rental.pricePerDay) : "—"}
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex gap-2">
                          <button onClick={() => edit(c)} className="px-3 py-1 rounded-md border border-gold/40 text-gold hover:bg-gold/10 transition text-xs whitespace-nowrap">تعديل</button>
                  ...
                  <tbody>
                    {[...rentals].reverse().map(r => (
                      <tr key={r.id} className="border-t border-border/50 hover:bg-secondary/30 transition">
                        ...
                        <td className="px-4 py-3 text-gold font-bold whitespace-nowrap">{formatEGP(r.totalPrice)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
...
function StatCard({ label, value, accent, highlight }: { label: string; value: string | number; accent?: "emerald" | "amber" | "rose" | "blue"; highlight?: boolean }) {
  const color = accent === "emerald" ? "text-emerald-400" : accent === "amber" ? "text-amber-400" : accent === "rose" ? "text-rose-400" : accent === "blue" ? "text-blue-400" : "text-foreground";
  return (
